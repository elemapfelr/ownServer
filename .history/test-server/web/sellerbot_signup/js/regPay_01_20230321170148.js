...
document.querySelector('#serviceVid').addEventListener('click', () => {
	...
	modalShow({
		id: 'youtube_00',
		content: `
        <iframe width="800" height="480" src="https://www.youtube.com/embed/4ZruNOiU07A" title="이용안내 셀러봇캐시 소개" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
        `,
	});
});
		...
		modalShow({
			...
			function: () => {
				...
				document.querySelector('#modalConfirm').addEventListener('click', () => {
					...
					document.querySelector('#bankBot .item').classList.remove('subscribing');
					regItems.bankBot = false;
					document.querySelector('.ticketFixed').classList.remove('active');
					ticketFixed_pc.classList.remove('show');
				});
			},
